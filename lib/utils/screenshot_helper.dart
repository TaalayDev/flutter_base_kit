...
class ScreenshotHelper {
  ...
  static Future<String?> takeAndSaveScreenshot({
    ...
    try {
      if (kIsWeb) {
        AppLogger.error('Screenshot capturing is not supported on web');
        return null;
      }

      // Capture screenshot
      final Uint8List? imageBytes = await controller.capture(delay: const Duration(milliseconds: 10), pixelRatio: 1.0);
      ...
      // Create file path
      final filePath = path.join(screenshotsDir.path, '$fileName.jpg');
  ...
  static Future<String?> generateThumbnail({
    ...
    try {
      ...
      final thumbnailPath = path.join(thumbnailsDir.path, '$thumbnailName.jpg');
